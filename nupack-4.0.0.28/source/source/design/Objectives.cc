...
void SimilarityObjective::initialize(Design const &design) {
    ...
    elements = vmap(component_names, [&](auto const &n) { return find_sequence_element(design, n); });
    // Check that total number of nucleotides match
    std::size_t nt = sum(elements, [](auto const el) {return fork(el, len);});
    NUPACK_REQUIRE(nt, ==, len(ref_seq), "reference sequence and design element for SimilarityObjective are different lengths", component_names, ref_seq, limits);
}
...
Defect SimilarityObjective::evaluate(Local const &env, Design const &design, uint depth, EnsemblePartition const &part, Weights const &weights, EngineObserver &) const {
    auto ref = ref_seq.begin();
    auto const matches = vmap(elements, [&](auto const &el) {
        auto const seq = fork(el, [&](auto const &x) {return x.to_sequence(design.sequence());});
        return vmap(seq, [&](Base s) -> uint {return is_base_specialization(*(ref++), s);});
    });
    NUPACK_ASSERT(ref == ref_seq.end());
    
    real const N = len(ref_seq), l = limits.first * N, u = limits.second * N;
