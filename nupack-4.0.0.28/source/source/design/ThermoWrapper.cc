...
real partition_function(Local const &env, ::nupack::Complex const &seqs,
    ...
    if (!engobs.slowdown)
        return thermo::dynamic_program<3, 0, 1>(env, seqs, models);

    decltype(thermo::dynamic_program<3, 0, 1>(env, seqs, models)) ret;
    auto time = time_it(engobs.slowdown, [&](){
        ret = thermo::dynamic_program<3, 0, 1>(env, seqs, models);
    });
...
std::pair<thermo::Tensor<real, 2>, real> pair_probability(Local const &env,
    ...
    if (!engobs.slowdown)
        return thermo::pair_probability<3, 0, 1>(env, seqs, models);

    decltype(thermo::pair_probability<3, 0, 1>(env, seqs, models)) ret;
    auto time = time_it(engobs.slowdown, [&]{
        ret = thermo::pair_probability<3, 0, 1>(env, seqs, models);
    });
...
real partition_function(Local const &env, ::nupack::Complex const &seqs, ThermoEnviron &t_env,
    ...
    return fork(std::get<0>(t_env.models).energy_model.ensemble_type(), [&](auto x) {
        ...
        decltype(thermo::dynamic_program<3, 0, 1>(env, seqs, t_env.doubled(), underlying_cache)) ret;
        if (!engobs.slowdown)
            return thermo::dynamic_program<3, 0, 1>(env, seqs, t_env.doubled(), underlying_cache);

        auto time = time_it(engobs.slowdown, [&]{
            ret = thermo::dynamic_program<3, 0, 1>(env, seqs, t_env.doubled(), underlying_cache);
        });
...
std::pair<thermo::Tensor<real, 2>, real> pair_probability(
    ...
    return fork(std::get<0>(t_env.models).energy_model.ensemble_type(), [&](auto x) {
        ...
        decltype(thermo::pair_probability<3, 0, 1>(env, seqs, t_env.doubled(), underlying_cache, obs)) ret;
        try {
            if (!engobs.slowdown) {
                ret = thermo::pair_probability<3, 0, 1>(env, seqs, t_env.doubled(), underlying_cache, obs);
            } else {
                auto time = time_it(engobs.slowdown, [&]{
                    ret = thermo::pair_probability<3, 0, 1>(env, seqs, t_env.doubled(), underlying_cache, obs);
                });
...
std::pair<thermo::Tensor<real, 2>, real> pair_probability(
    ...
    decltype(thermo::bonus_pair_probability<3, 0, 1>(env, seqs, models, False(), obs, pairing, use_B)) ret;

    if (!engobs.slowdown) {
        ret = thermo::bonus_pair_probability<3, 0, 1>(env, seqs, models, False(), obs, pairing, use_B);
    } else {
        auto time = time_it(engobs.slowdown, [&]{
            ret = thermo::bonus_pair_probability<3, 0, 1>(env, seqs, models, False(), obs, pairing, use_B);
        });
