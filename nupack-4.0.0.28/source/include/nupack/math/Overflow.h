...
template <class T> using exponent_t = copy_qualifier<T, int_of_size<sizeof(T)>>;
...
decltype(auto) mantissa_at(V &&v, simd::Chunk<N> i) {
    return simd::load(i, std::addressof(*(begin_of(v) + i.value)));
}

template <class V, class I, NUPACK_IF(is_scalar_range<V>)>
simd::Zero exponent_at(V &&v, I i) {return {};}
...
decltype(auto) mantissa_at(V &&v, simd::Chunk<N> i) {
    return simd::load(i, &first_iter(begin_of(v))[i.value]);
}

template <class V, int N, NUPACK_IF(is_compound_range<V>)>
decltype(auto) exponent_at(V &&v, simd::Chunk<N> i) {
    return simd::load(i, &second_iter(begin_of(v))[i.value]);
}

/******************************************************************************************/

template <class P, class T=simd::Zero>
...
template <class P, class T=simd::Zero, NUPACK_IF(!is_class<decay<decltype(*std::declval<P>())>>)>
decltype(auto) mantissa(P &&p, T={}) {return *p;}

template <class P, class T=simd::Zero, NUPACK_IF(!is_class<decay<P>>)>
remove_rref<P &&> mantissa(P &&p, T={}) {return fw<P>(p);}

template <class P, class T=simd::Zero, NUPACK_IF(!is_class<decay<decltype(*std::declval<P>())>>)>
T exponent(P const &, T hint={}) {return hint;}

template <class P, class T=simd::Zero, NUPACK_IF(!is_class<P>)>
T exponent(P const &, T hint={}) {return hint;}
...
#       ifndef NUPACK_NO_SIMD
            ...
            auto const first = std::min(i + next_alignment(std::addressof(*(begin_of(out) + i))), stop);
            for (; i < first; ++i) 
                *(begin_of(out) + i) = f(i).first;

            constexpr auto Z = simd::optimal_size<value_type_of<O>>;
            for (; i + Z <= stop; i += Z)
                simd::store(f(Chunk<Z>(i)).first, std::addressof(*(begin_of(out) + i)));

#       endif
        for (; i < stop; ++i) {
            *(begin_of(out) + i) = f(i).first;
            static_assert(std::is_same_v<std::decay_t<decltype(f(i).second)>, simd::Zero>);
        }
    } else {
...
#       ifndef NUPACK_NO_SIMD
            ...
            static_assert(!is_same<value_type_of<decltype(m)>, double>, "this is screwed up in simdpp right now");
...
template <class T>
struct SingleDispatch<overflow<T>> {
    using O = std::decay_t<T>;

    static overflow<O> exp(overflow<T> const &t) noexcept {return {std::exp(T(t)), 0};}
    static O log(overflow<T> const &t) noexcept {return std::log(t.first) + t.second * LogOf2;}
    static overflow<O> reciprocal(overflow<T> const &t) noexcept {return {1/t.first, -t.second};}
};


template <class T, class U>
struct DoubleDispatch<overflow<T>, U> {
    using O = std::decay_t<T>;
    static_assert(std::is_floating_point_v<U>);

    static overflow<O> multiplies(overflow<T> const &t, U const &u) noexcept {
        return overflow<O>(t.first * u, t.second);
    }
};

template <class T, class U>
struct DoubleDispatch<U, overflow<T>> {
    using O = std::decay_t<T>;
    static_assert(std::is_floating_point_v<U>);
    
    static overflow<O> multiplies(U const &u, overflow<T> const &t) noexcept {
        return overflow<O>(u * t.first, t.second);
    }
    static void set_logarithm(U &u, overflow<T> const &t) noexcept {
        u = std::log(static_cast<std::common_type_t<T, U>>(t.first)) + t.second * LogOf2; // use higher precision type
    }
};

template <class T, class U>
struct DoubleDispatch<overflow<T>, overflow<U>> {
    using O = std::decay_t<std::common_type_t<T, U>>;
    
    static overflow<O> multiplies(overflow<T> const &t, overflow<U> const &u) noexcept {
        return overflow<O>(t.first * u.first, t.second + u.second);
    }

    static overflow<O> plus(overflow<T> const &t, overflow<U> const &u) noexcept {
        if (t.second < u.second)
             return overflow<O>(u.first + std::ldexp(t.first, t.second-u.second), u.second);
        else return overflow<O>(t.first + std::ldexp(u.first, u.second-t.second), t.second);
    }
};
