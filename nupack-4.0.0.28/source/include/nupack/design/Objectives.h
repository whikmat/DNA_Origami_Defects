...
struct SimilarityObjective {
    ...
    SimilarityObjective(vec<string> names, vec<Sequence> seqs, std::pair<real, real> lims) :
            component_names(names), ref_seq(join(seqs)), limits(lims) {}
    ...
    NUPACK_REFLECT(SimilarityObjective, component_names, ref_seq, limits, elements);
};
