        ...
        return [&, i, j, A] (auto &&x, auto const &x2) {
            if (i+15 < j) {
                ...
                simd::map(x, K, [&](auto k) {
                    ...
                    auto const e = simd::negate(simd::max_of(exponent_at(x2, k-2), exponent_at(y1, j+3-k), exponent_at(y2, k+i-3)));
                    auto const m = A.plus(
                    ...
                    return std::make_pair(m, simd::negate(e));
                });
            }
        ...
        return [&, i, j, A] (auto &&x, auto const &x2) {
            ...
            if (i+6 < m && n+6 <= j) {
                ...
                simd::map(x, 10, mm.first, [&](auto s) {
                    ...
                    auto const e = simd::negate(simd::max_of(exponent_at(x2, s-2), exponent_at(y1, j+3-s), exponent_at(y2, s+i-3)));
                    auto const m = A.plus(
                ...
                simd::map(x, r, w, [&](auto s) {
                    auto const e = simd::negate(simd::max_of(exponent_at(y2, s+i-3), exponent_at(x2, s-2)));
                    auto const m = A.plus(
                        /*C5*/ A.ldexp(A.times(mantissa_at(x2, s-2), t.int_scale(s-2)), exponent_at(x2, s-2) + e),
                ...
                simd::map(x, w, r, [&](auto s) {
                    auto const e = simd::negate(simd::max_of(exponent_at(y1, j+3-s), exponent_at(x2, s-2)));
                    auto const m = A.plus(
                        /*C5*/ A.ldexp(A.times(mantissa_at(x2, s-2), t.int_scale(s-2)), exponent_at(x2, s-2) + e),
