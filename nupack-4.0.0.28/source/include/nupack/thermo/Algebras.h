...
template <class M>
auto expression(M &&mantissa) {return expression(fw<M>(mantissa), simd::always_zero);}
...
struct ForwardAlgebra {
    ...
    template <class T, class ...Ts> auto product(T const &t, Ts const &...ts) const {
        return expression([=] (auto hint) {
            static_assert(is_same<decltype(hint), simd::Zero> || is_signed<decltype(hint)>, "");
    ...
    template <class T, class F> auto total(T const &t, F &&f) const {
        return expression([=] (auto hint) {
            ...
            for (auto i : t) {
                auto elem = f(i);
                ret = Rig::plus()(ret, Rig::ldexp()(mantissa(elem, hint), exponent(elem, hint)));
            }
            return ret;
        });
    }
